...
A preprocessing script was then used to create segmentation masks from these annotations in order to have a ground truth for training. Since the original images have varying length, a sliding window approach was used next to crop squared windows from the images. If the length of the image is larger than the height then the windows size is _height x height_ whereas if the height is larger than the length the windows size is _length x length_ and croppping is done around the middle of the heart. These windows then correspond to the training and test set. An example of an image used for training along with its corresponding mask can be seen here:
